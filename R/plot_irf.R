...
#'
#' @export
individual_lp_irf_plot = function(
  irf,                 # var_irf object
  shock.var,            # string: name of variable to treat as the shock
  response.var,         # string: name of variable to treat as the response
  title,                # string: title of the chart
  ...
  plotdata = irf %>% dplyr::filter(shock == shock.var & target == response.var) %>%
    dplyr::select(horizon, coef, ci.lower, ci.upper)

  # plot
  irf.plot <- plotdata  %>%
    ggplot2::ggplot(ggplot2::aes(x=horizon, y=coef, ymin=ci.lower, ymax=ci.upper)) +
    ggplot2::geom_hline(yintercept = 0, color="red") +
    ggplot2::geom_ribbon(fill="grey", alpha=0.2) +
    ggplot2::geom_line() +
    ggplot2::theme_light() +
...
lp_irf_plot = function(
  ...
  # set shocks and responses
  if(is.null(shocks)){
    shocks = unique(irfs$shock)
  }
  if(is.null(responses)){
    responses = unique(irfs$target)
  }

  # generate plots
  plot.names = tidyr::expand_grid(shock = shocks, target = responses)
  plots = split(plot.names, seq(nrow(plot.names))) %>%
    purrr::map(.f = function(x){

      chart =
        individual_lp_irf_plot(
          irfs,
          shock.var = x$shock,
          response.var = x$target,
          title = paste0(x$target, ' response to ', x$shock, ' shock'),
          ylab = '')
